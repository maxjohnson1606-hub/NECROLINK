...
export const Members = () => {
  ...
  const MemberCard = ({ member, index }) => {
    const online = isOnline(member.game_name) || isOnline(member.name);
    return (
      <Link to={`/members/${encodeURIComponent(member.game_name)}`} className="block">
        <motion.div
          ...
          className={`bg-darknet-surface border p-6 hover:border-neon-blue/50 transition-all cursor-pointer ${
            online ? 'border-green-400/40' : 'border-border-DEFAULT'
          }`}
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="relative flex-shrink-0">
              <div className={`w-20 h-20 border-2 overflow-hidden ${
                online ? 'border-green-400' : 'border-neon-blue'
              }`}>
                {member.avatar_url ? (
                  <img src={member.avatar_url} alt={member.name} className="w-full h-full object-cover" />
                ) : (
                  <div className="w-full h-full bg-darknet-terminal flex items-center justify-center">
                    <Zap className={`w-8 h-8 ${online ? 'text-green-400' : 'text-neon-blue'}`} />
                  </div>
                )}
              </div>
              {online && (
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-1 flex-wrap">
                <h3 className="font-heading text-lg font-bold text-white uppercase">{member.name}</h3>
                {online && <OnlineDot size="sm" showLabel />}
              </div>
    ...
    <div className="min-h-screen bg-darknet-bg py-16 px-4">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="text-center mb-12">
          ...
          <p className="font-body text-lg text-text-secondary tracking-wide mb-4">
